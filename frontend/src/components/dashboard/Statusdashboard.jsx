...
const Statusdashboard = () => {
  ...
  const fetchStatusCount=async()=>{
    ...
    try{
      ...
      const openstatusCount=response.data.filter((ticket)=>ticket.status==="open").length
      setOpenCount(openstatusCount)
      const closeStatusCount=response.data.filter((ticket)=>ticket.status==="close").length
            ...
            <Link to='/tickets/open' style={{ textDecoration: 'none'}}  >
              <div className="card border rounded bg-light " >
                <div className="card-body d-flex flex-column align-items-center"  >
                  ...
                  <h5 className="card-title fw-bold text-center"> <span className='text-danger'>{openCount}</span> Open Ticket </h5>
                  <p className="text-center text-muted small">Manage and view open tickets</p>
                </div>
              </div>
            </Link>
          </div>
            ...
            <Link to='/tickets/close' style={{ textDecoration: 'none' }} >
              <div className="card border  rounded bg-light">
                <div className="card-body d-flex flex-column align-items-center">
                  ...
                  <h5 className="card-title fw-bold text-center"> <span className='text-success'>{closeCount}</span> Close Ticket</h5>
                  <p className="text-center text-muted small">Close resolved tickets</p>
                </div>
              </div>
            </Link>
          </div>
